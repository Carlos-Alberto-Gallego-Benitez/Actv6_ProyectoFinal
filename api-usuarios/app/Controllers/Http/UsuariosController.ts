...
export default class UsuariosController {
  ...
  //estado de la api
    ...
    public async login({response, request}: HttpContextContract){
      ...
      if(base64.decode(y.password) === query.passw){
        ...
        var token = sign({ data: {user: query.user, passw: y.password, name: y.nombre}, exp: Math.floor(Date.now() / 1000) + (20 * 20) }, privateKey, { algorithm: 'RS256'});
        salida = {token: token, user: query.user, name: y.nombre }
  ...
  public async insertUser({response, request}: HttpContextContract){
      //entrada con los datos del usuario a insertar
      let { query } = request.all();
      query = JSON.parse(base64.decode(query));
      
      let sql = `INSERT INTO jsonplaceholder.usuario (nombre, correo, celular, password)VALUES('${query.nombre}', '${query.correo}', '${query.celular}', '${query.password}')`;
